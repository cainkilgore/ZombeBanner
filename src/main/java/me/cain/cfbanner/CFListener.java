	...
	public void onPlayerJoin(PlayerJoinEvent Event) {
		...
		if (!CFBanner.PermissionCheck("cf.allowfly", player)) {
			...
			if (CFBanner.config.getBoolean("config.showmessages", true)) {
				player.sendMessage(ChatColor.GREEN + "[CFBanner] "
						+ ChatColor.WHITE + "You are allowed to fly!");
			}
		}
		...
		if (!CFBanner.PermissionCheck("cf.allowcheat", player)) {
			...
			if (CFBanner.config.getBoolean("config.showmessages", true)) {
				player.sendMessage(ChatColor.GREEN + "[CFBanner] "
						+ ChatColor.WHITE + "You are allowed to cheat!");
			} else {
				return;
			}
		...
		if (!CFBanner.PermissionCheck("cf.allowfly", player)
				&& !CFBanner.PermissionCheck("cf.allowcheat", player)) {
			if (CFBanner.config.getBoolean("config.showmessages", true)) {
				player.sendMessage(ChatColor.DARK_RED
						+ CFBanner.config.getString("config.cfdisabledmessage",
